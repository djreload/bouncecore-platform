param(
  ...
  [string]$SecondaryUserEmail = $env:STREAM_SMOKE_SECONDARY_USER_EMAIL,
...
function Create-TempKey([string]$Email) {
  ...
  Write-Host "Temporary stream key created for ${Email}: $($tempKey.fingerprint)"

  return $tempKey
}

function Revoke-TempKey($TempKey) {
  if (-not $TempKey -or -not $TempKey.keyId) {
    return
  }
