...
STREAM_PROVIDER="$(prompt "Stream provider" "stream-core")"
INTERNAL_TASK_TOKEN="$(prompt_optional_secret "Internal task token")"
INTERNAL_TASK_TOKEN="${INTERNAL_TASK_TOKEN:-$(generate_secret)}"
ENABLE_STREAM_CORE="$(prompt "Start embedded stream core service now? Use n if another service owns stream ports" "y")"
ENABLE_MEDIA_GATEWAY="$(prompt "Start MediaMTX RTMP/HLS gateway now? Use n if stream ports are already in use" "y")"
ENABLE_TRANSCODER="$(prompt "Start FFmpeg adaptive HLS transcoder now? Requires the MediaMTX gateway" "y")"
...
MEDIA_GATEWAY_INTERNAL_HLS_URL=$MEDIA_GATEWAY_INTERNAL_HLS_URL
STREAM_CORE_SECONDARY_PLAYBACK_URL=$STREAM_CORE_SECONDARY_PLAYBACK_URL
...
RESTREAMER_SECONDARY_CONTAINER=bouncecore-media-restreamer-secondary
