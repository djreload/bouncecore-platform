...
ExecStart=/usr/bin/env bash -lc 'bash scripts/backup-instance.sh --env-file "$ENV_FILE" --compose-file "$COMPOSE_FILE" --backup-root "$BACKUP_ROOT" --retention-days "$RETENTION_DAYS"'
SERVICE
