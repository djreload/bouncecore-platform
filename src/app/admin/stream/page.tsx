...
import type { AdminStreamChannelRow } from "@/app/admin/stream/state";
...
export default async function AdminStreamPage() {
  ...
  const { channels, provider } = await getAdminStreamControlData();
  ...
  return (
    ...
    >
      <AdminStreamControlPanel channels={channelRows} provider={provider} />
    </AdminShell>
  );
}
