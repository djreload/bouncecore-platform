...
test("sheep throw settings input converts admin minutes to seconds", () => {
  assert.deepEqual(normalizeSheepThrowSettingsInput({ enabled: false, cooldownMinutes: "2.5" }), {
    ...defaultSheepThrowSettings,
    enabled: false,
    cooldownSeconds: 150
  });
...
test("sheep throw settings reject impossible cooldown values", () => {
  assert.throws(() => normalizeSheepThrowSettingsInput({ enabled: true, cooldownMinutes: "-1" }), /between 0 and 1440/);
  assert.throws(() => normalizeSheepThrowSettingsInput({ enabled: true, cooldownMinutes: "1441" }), /between 0 and 1440/);
});
