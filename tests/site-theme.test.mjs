...
test("shared visual system styling is driven by theme tokens", () => {
  ...
  assert.match(globalsCss, /--bc-gradient-brand:/);
  assert.match(globalsCss, /--bc-field-surface:/);
  ...
  assert.match(globalsCss, /\.bc-button-ghost/);
  assert.match(globalsCss, /\.bc-button-dark/);
  assert.match(globalsCss, /input\[type="file"\]::file-selector-button/);
  assert.match(globalsCss, /:where\(\.bc-app-shell, \.bc-dashboard-shell\) th/);
  assert.match(globalsCss, /@keyframes bc-background-drift/);
  ...
  assert.match(buttonSource, /bc-button-ghost/);
  assert.match(buttonSource, /bc-button-dark/);
  assert.doesNotMatch(buttonSource, /#00d5ff/);
  assert.doesNotMatch(buttonSource, /#ff2bd6/);
  assert.doesNotMatch(buttonSource, /shadow-\[/);
});
