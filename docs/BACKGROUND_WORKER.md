...
The worker uses the same `DATABASE_URL`, push encryption key, and optional Expo access token as the main app. A Rave War alert is created once per operator and match revision after at least 150 seconds without server activity. Further alerts are possible only after the match revision changes and then stalls again.
