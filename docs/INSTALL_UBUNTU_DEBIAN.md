...
1. Create a Business type Meta app, add Facebook Login for Business, and enable access to the Live Video API.
...
4. Create a Facebook Login for Business User access token configuration containing Page assets and `pages_show_list`, `pages_read_engagement`, and `pages_manage_posts`. Copy its Configuration ID.
5. Request or complete Meta App Review for those permissions when users outside the app's assigned roles will connect Pages.
6. Keep `PUSH_TOKEN_ENCRYPTION_KEY` configured so Page tokens and generated secure ingest URLs can be encrypted.
7. Save the Meta app ID, secret, and Configuration ID under Admin -> Stream, or configure:

```env
FACEBOOK_APP_ID="your-meta-app-id"
FACEBOOK_APP_SECRET="your-meta-app-secret"
FACEBOOK_LOGIN_CONFIG_ID="your-facebook-login-for-business-configuration-id"
```

8. Set the destination provider to Facebook, enter the destination Page ID, enable and save it, then press `Connect Facebook Page`.
9. Have an administrator of that Facebook Page complete the authorization. This can be a different person from the Bouncecore owner.
